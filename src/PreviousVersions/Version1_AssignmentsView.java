...
public class Version1_AssignmentsView {
	private Object[] headerLabels;
	private Object[][] rows;
	private JFrame frame;
	public Version1_AssignmentsView() {
		// read the input file
		try {
...
//		frame.getContentPane().add(new NavigationButtonBanner(),BorderLayout.NORTH);
		...
		for (int i = 0; i < headerLabels.length; i++) {
			...
			if (i == 0) {
				// if we are setting the first column, this is the student column, and we want it to have different behavior from the rest of the columns
				table.getColumn(colTitle).setCellEditor(new ButtonEditor_Student(new JCheckBox()));
			} else {
				// all other columns besides the student column have the same behavior
				table.getColumn(colTitle).setCellEditor(new ButtonEditor_Grade(new JCheckBox()));
			}
		}
	...
	public static void main(String[] args) {
		Version1_AssignmentsView av = new Version1_AssignmentsView();
	}
}
